...
class GeneratedMNISTDataset(Dataset):
    """
    PyTorch Dataset for generated MNIST-like images with pseudo-labels.

    This dataset wraps images generated by a diffusion model and associates
    each image with a pseudo-label predicted by a pretrained classifier.
    Samples without a saved image are automatically filtered out.

    Each item returned consists of:
        - image: a grayscale tensor or PIL image (depending on the transform)
        - label: an integer digit label in [0–9] or a special 'IDK' label
                 indicating classifier uncertainty

    Args:
        results_list (list of dict): list of evaluation results produced during image generation.
            Each dictionary is expected to contain at least:
                - 'img_path': path to the saved generated image (or None)
                - 'classifier_label': pseudo-label predicted by a pretrained classifier
        transform (callable, optional): optional transform applied to each image (e.g., normalization,
            resizing, conversion to tensor).
    """
    ...
    def __len__(self):
        """Returns the total number of valid generated samples in the dataset."""
    ...
    def __getitem__(self, idx):
        ...
        Returns (via __getitem__):
            tuple (image, label), where:
                    - image is a grayscale image (PIL Image or tensor if transformed)
                    - label is the pseudo-label predicted by the pretrained classifier
                    (integer in [0–9] or 'IDK')
        """
